...
The collection pipeline supports **contribution extent and contextual quality**. It separates acquisition, validation, recovery, and database persistence so the analytical results are built on identifiable collection stages.
...
**Workflow:** discovery → profiles → weekly activity → repositories → pull requests → validated PostgreSQL records → analytical features.

Weekly activity supplies pull-request references. Repository enrichment provides context; detail collection retains status, additions, deletions, commits, changed files, and comments. Observation dates distinguish a collected status snapshot from a final outcome. Private feature assembly controls which information was available at each modeling cutoff.
...
Pull-request structural records feed [extent preparation](extent_model.py), [contribution archetypes](contribution_archetypes.py), and the private feature assembly for the [quality model](quality_model.py).
